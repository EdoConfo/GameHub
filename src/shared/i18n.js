// Interface language. One dictionary per language, flat dotted keys.
//
// Rules of the house:
//   - every user-visible string lives here, never inline in a screen
//   - t('key', { name }) fills {name} placeholders
//   - a missing key falls back to Italian, then to the key itself, so a
//     half-translated language never renders an empty screen
//
// Word packs are NOT here: they carry their own languages (see packStore.js).
import * as storage from './storage.js'

const KEY = 'lang'

// Order matters: the Lingua bead in settings cycles through this list.
export const LANGS = ['it', 'en']
export const LANG_NAMES = { it: 'Italiano', en: 'English' }
// For toLocaleDateString and localeCompare.
export const LOCALES = { it: 'it-IT', en: 'en-GB' }

const DICT = {
  it: {
    'common.back': 'Indietro',
    'common.forward': 'Avanti',
    'common.cancel': 'Annulla',
    'common.save': 'Salva',
    'common.delete': 'Elimina',
    'common.edit': 'Modifica',
    'common.done': 'Fatto',
    'common.new': 'Nuovo',
    'common.menu': 'Menu',
    'common.restore': 'Ripristina',
    'common.whatItDoes': 'Cosa fa',

    'hub.players': 'Giocatori',
    'hub.settings': 'Impostazioni',

    'settings.theme': 'Tema',
    'settings.dark': 'Scuro',
    'settings.light': 'Chiaro',
    'settings.theme.system': 'Sistema',
    'settings.theme.light': 'Chiaro',
    'settings.theme.dark': 'Scuro',
    'settings.language': 'Lingua',
    'settings.offline': 'Offline',
    'settings.offlineSub': 'Installata · funziona senza rete',
    'settings.version': 'Versione',
    'settings.wordsRev': '{game} r{n}',
    'update.ready': 'Nuova versione',
    'update.words': 'Parole aggiornate',
    'update.wordsFailed': 'Parole non aggiornate: {reason}',
    'storage.full': 'La memoria dell\u2019app è piena: alcune modifiche non verranno salvate. Le foto dei giocatori sono la parte più pesante.',
    'storage.fullShort': 'memoria del telefono piena',
    'mw.opt.baseMissing': 'Base da scaricare: serve la rete una volta',
    'update.action': 'Aggiorna',
    'settings.install': 'Installa',
    'settings.installSub': 'Aggiungila alla Home',
    'install.title': 'Aggiungila alla Home',
    'install.why': 'Diventa un\u2019app: icona sua, niente barre del browser, e resta a posto anche senza rete.',
    'install.ios.1': 'Tocca Condividi, in fondo allo schermo.',
    'install.ios.2': 'Scorri e scegli Aggiungi a Home.',
    'install.ios.3': 'Conferma con Aggiungi, in alto a destra.',
    'install.android.1': 'Apri il menu del browser, i tre puntini in alto.',
    'install.android.2': 'Scegli Installa app, o Aggiungi a schermata Home.',
    'install.desktop.1': 'Nella barra degli indirizzi cerca l\u2019icona di installazione.',
    'install.desktop.2': 'In alternativa, dal menu del browser scegli Installa.',
    'install.macos.1': 'Apri il menu Condividi, nella barra degli strumenti.',
    'install.macos.2': 'Scegli Aggiungi al Dock.',
    'install.noneTitle': 'Qui non si pu\u00f2 installare',
    'install.none.1': 'Questo browser non sa installare le app web.',
    'install.none.2': 'Nessun problema: qui dentro funziona lo stesso, anche senza rete. Per averla come app, aprila con Chrome, Edge o Safari.',

    'players.profile': 'Profilo',
    'players.addPlayer': 'Aggiungi giocatore',
    'players.newTitle': 'Nuovo giocatore',
    'players.editTitle': 'Modifica giocatore',
    'players.loadPhoto': 'Carica foto',
    'players.removePhoto': 'Rimuovi foto',
    'players.namePlaceholder': 'Nome',
    'players.color': 'Colore',
    'players.emojiOptional': 'Emoji (opzionale)',
    'players.nameRequired': 'Serve un nome.',
    'players.badImage': 'Immagine non valida',

    'player.perGame': 'Per gioco',
    'player.recent': 'Attività recente',
    'player.emptyFeed': 'Nessuna partita ancora. Gioca per riempire il feed!',
    'player.win': 'Vinta',
    'player.loss': 'Persa',
    'player.deleteTitle': 'Eliminare {name}?',
    'player.deleteBody': 'Il giocatore viene rimosso dal roster. Le statistiche registrate restano nei giochi.',

    'stats.title': 'Statistiche',
    'stats.played': 'Partite',
    'stats.won': 'Vittorie',
    'stats.winRate': 'Win rate',
    'stats.player': 'Giocatore',
    'stats.playedShort': 'Giocate',
    'stats.wonShort': 'Vinte',
    'stats.empty': 'Ancora nessuna partita registrata. Gioca per vedere le statistiche!',
    'stats.sub': '{played} partite · {won} vinte · {pct}%',
    'stats.emptyTitle': 'Nessuna partita',
    'stats.emptySub': 'Gioca per riempire la classifica',

    'time.now': 'ora',
    'time.min': '{n}m fa',
    'time.hour': '{n}h fa',
    'time.day': '{n}g fa',

    'table.seatFree': 'Posto libero',
    'table.drawerToggle': 'Apri o chiudi il cassetto',
    'table.sheetClose': 'Chiudi',
    'table.free': 'libero',
    'table.seat': 'posto',
    'table.seats': 'posti',
    'table.addSeat': 'Sedia',
    'table.addPlayer': 'Giocatori',
    'table.start': 'Inizia',
    'table.needSeats': 'Servono almeno {n} posti',
    'table.whoSits': 'Chi si siede?',
    'table.pickForSeat': 'Scegli chi siede qui, o lascialo libero: chi lo trova si presenta quando riceve il telefono.',
    'table.pickProfile': 'Scegli un profilo o creane uno nuovo.',
    'table.allSeated': 'Tutti i profili sono già al tavolo: creane uno nuovo.',
    'table.empty': 'Svuota il tavolo',
    'table.emptySure': 'Svuota davvero',
    'table.playersHint': 'Tocca chi si siede: il pannello resta aperto, così li aggiungi tutti in fila. L’ordine lo sistemi dopo, sul tavolo.',
    'table.free.action': 'Libera',
    'table.remove': 'Togli',

    'packs.title': 'Parole',
    'packs.section': 'Pacchetti',
    'packs.new': '+ Nuovo pacchetto',
    'packs.newTitle': 'Nuovo pacchetto',
    'packs.newSub': 'Scrivi le tue parole',
    'packs.icon': 'Icona',
    'packs.addRow': '+ Riga',
    'packs.dropRow': 'Togli la riga',
    'packs.dropRowSure': 'Togli davvero la riga',
    'packs.rowCount': '{n} {unit}',
    'packs.halfRow': '1 riga da completare',
    'packs.halfRows': '{n} righe da completare',
    'packs.halfRowStop': 'Completa o togli la riga lasciata a metà.',
    'packs.halfRowsStop': 'Completa o togli le {n} righe lasciate a metà.',
    'packs.inPlay': 'Usa in partita',
    'packs.editTitle': 'Modifica pacchetto',
    'packs.namePlaceholder': 'Nome pacchetto',
    'packs.use': 'Usa {name}',
    'packs.tagCustom': ' · tuo',
    'packs.tagLocked': ' · nascosto',
    'packs.locked': '{n} {unit}. Le parole del pacchetto di base restano nascoste: così nessuno al tavolo le conosce prima di giocare.',
    'packs.lockedEdit': 'Il pacchetto di base non si modifica.',
    'packs.tagModified': ' · modificato',
    'packs.deleteSure': 'Elimina davvero',
    'packs.deleted': 'Eliminato “{name}”',
    'packs.restored': 'Ripristinato com’era',
    'packs.saved': 'Salvato “{name}” ({n} {unit})',
    'packs.added': 'Aggiunto “{name}” ({n} {unit})',
    'packs.nameRequired': 'Dai un nome al pacchetto.',
    'packs.emptyText': 'Il testo è vuoto.',
    'packs.badJson': 'JSON non valido. Controlla la sintassi.',
    'packs.missingField': 'JSON senza campo "{field}" valido.',
    'packs.notFound': 'Pacchetto non trovato.',
    'packs.unit.items': 'voci',

    'mw.name': 'Mr White',
    'mw.description': 'Trova l’impostore che non conosce la parola.',
    'mw.menu.play': 'Gioca',
    'mw.menu.playSub': 'Nuova partita',
    'mw.menu.words': 'Parole',
    'mw.menu.wordsSub': 'Pacchetti e coppie',
    'mw.menu.rules': 'Come si gioca',
    'mw.menu.rulesSub': 'Regole e ruoli',
    'mw.menu.stats': 'Statistiche',
    'mw.menu.statsSub': 'Partite e vittorie',

    'mw.unit.pairs': 'coppie',
    'mw.packsHelp': 'Coppie di parole: quella dei civili e una simile per gli Undercover. Tocca un pacchetto per rinominarlo o cambiarne le coppie. Quali usare lo scegli al tavolo, prima di ogni partita.',
    'mw.packsPlaceholder': 'Una coppia per riga:\ncane,lupo\npizza,focaccia\n\n…oppure JSON: { "pairs": [ { "civilian": "cane", "undercover": "lupo" } ] }',
    'mw.packsEmpty': 'Nessuna coppia valida. Formato: "parola,parola-simile" per riga.',

    'mw.role.civile': 'Civile',
    'mw.role.undercover': 'Undercover',
    'mw.role.mrwhite': 'Mr White',
    'mw.roles.civili': 'Civili',
    'mw.roles.impostori': 'Impostori',

    'mw.rules.rolesSection': 'Ruoli',
    'mw.rules.civili': 'Ricevono la parola segreta.',
    'mw.rules.undercover': 'Ricevono una parola simile ma diversa.',
    'mw.rules.mrwhite': 'Non riceve nessuna parola: deve fingere di saperla.',
    'mw.rules.goddess': 'Dea della giustizia',
    'mw.rules.goddessDesc': 'Ce n’è una per giro, estratta a caso tra chi è ancora in gioco. Se al voto c’è un pareggio, decide lei chi eliminare. Al giro dopo ne esce un’altra.',
    'mw.rules.countSection': 'Quanti impostori',
    'mw.rules.minority': 'Sempre in minoranza',
    'mw.rules.minorityDesc': 'I civili devono essere più degli impostori (Undercover + Mr White): con 3–4 giocatori 1 impostore, con 5–6 fino a 2, con 7–8 fino a 3, e così via.',
    'mw.rules.suggested': 'Consigliati',
    'mw.rules.suggestedDesc': 'Circa 4 impostori ogni 10 giocatori: un Mr White (due da 11 in su), il resto Undercover.',
    'mw.rules.flowSection': 'Come si svolge',
    'mw.rules.step0': '0 · Il tavolo',
    'mw.rules.step0Desc': 'Disponete i posti come siete seduti. Chi trova un posto libero si presenta quando riceve il telefono.',
    'mw.rules.step1': '1 · Distribuzione',
    'mw.rules.step1Desc': 'Il telefono fa il giro del tavolo: ognuno vede in privato la sua parola (o scopre di essere Mr White).',
    'mw.rules.step2': '2 · Indizi',
    'mw.rules.step2Desc': 'A turno, in senso orario, ognuno dice a voce una parola collegata alla propria. Non scriverla. Non comincia mai un Mr White.',
    'mw.rules.step3': '3 · Votazione',
    'mw.rules.step3Desc': 'Discutete ed eliminate un sospetto toccandolo sul tavolo. Si scopre il suo ruolo. In caso di pareggio decide la Dea della giustizia.',
    'mw.rules.winSection': 'Chi vince',
    'mw.rules.winCivili': 'Se eliminano tutti gli impostori (Undercover + Mr White).',
    'mw.rules.winImpostori': 'Se resistono finché resta un solo civile.',
    'mw.rules.winMrWhite': 'Se, una volta eliminato, indovina la parola dei civili.',

    'mw.setup.min3': 'Servono almeno 3 giocatori.',
    'mw.setup.max20': 'Massimo 20 giocatori.',
    'mw.setup.needImpostor': 'Serve almeno un impostore.',
    'mw.setup.tooMany': 'Troppi impostori: i civili devono essere di più.',
    'mw.setup.pickPack': 'Scegli almeno un pacchetto di parole',
    'mw.opt.less': 'Meno {label}',
    'mw.opt.more': 'Più {label}',
    'mw.opt.civili': 'Civili',
    'mw.opt.characters': 'Personaggi',
    'mw.opt.charactersHint': 'I civili sono tutti gli altri: quelli che restano dopo gli impostori. Tocca ? per sapere cosa fa un ruolo.',
    'mw.opt.words': 'Parole',
    'mw.opt.choose': 'Scegli',
    'mw.opt.nPacks': '{n} pacchetti',
    'mw.opt.packsHint': 'Scegli da quali pacchetti pescare la coppia. Si creano e si modificano da “Parole” nel menu del gioco.',
    'mw.opt.nPairs': '{n} coppie',

    'mw.extra.meme': 'Mr Meme',
    'mw.extra.memeDesc': 'A ogni giro di indizi tocca a uno a caso: descrive la sua parola solo a gesti, senza parlare.',
    'mw.extra.lovers': 'Innamorati',
    'mw.extra.loversDesc': 'Due giocatori si innamorano all’inizio, e lo sanno. Se uno viene eliminato l’altro lo segue, e si scopre solo in quel momento.',
    'mw.extra.revenger': 'Vendicatore',
    'mw.extra.revengerDesc': 'Quando viene eliminato si porta via un altro giocatore, scelto da lui. Vale una volta sola.',
    'mw.extra.needPlayers': 'Serve da {n} giocatori in su.',
    'mw.extra.section': 'In più',

    'mw.deal.loverOf': 'Sei innamorato di {name}.',
    'mw.deal.revenger': 'Sei il Vendicatore: se ti eliminano, ne porti via uno con te.',

    'mw.play.meme': 'Tocca a {name}: descrive la sua parola a gesti, senza parlare.',
    'mw.play.memeNote': 'gesti',
    'mw.play.loversOut': '{a} e {b} erano innamorati.',

    'mw.revenge.kicker': 'La vendetta di {name}',
    'mw.revenge.hint': 'Tocca sul tavolo chi si porta via.',
    'mw.revenge.pick': 'Scegli chi',
    'mw.revenge.confirm': 'Porta via {name}',
    'mw.revenge.note': 'vendetta',

    'mw.deal.counter': 'parole',
    'mw.deal.passTo': 'Passa il telefono a',
    'mw.deal.onlyYou': 'Solo tu devi vedere lo schermo.',
    'mw.deal.seeWord': 'Vedi la tua parola',
    'mw.deal.youAreMrWhite': 'Sei Mr White',
    'mw.deal.mrWhiteHint': 'Non conosci la parola: fingi di saperla.',
    'mw.deal.yourWord': 'La tua parola è',
    'mw.deal.hideAndStart': 'Nascondi e iniziate',
    'mw.deal.hideAndPass': 'Nascondi e passa',

    'mw.join.freeSeat': 'Posto libero',
    'mw.join.who': 'Chi sei?',
    'mw.join.pickOrCreate': 'Prima della parola, presentati: tocca il tuo profilo o creane uno.',
    'mw.join.createOnly': 'Prima della parola, crea il tuo profilo.',
    'mw.join.createMine': 'Crea il mio profilo',

    'mw.goddess.kicker': 'Dea della giustizia',
    'mw.goddess.short': 'Dea',
    'mw.goddess.hint': 'Se al voto c’è un pareggio, decide lei chi eliminare.',
    'mw.goddess.start': 'Iniziamo',
    'mw.goddess.round': '{name} è la Dea della giustizia di questo giro.',

    'mw.play.inGame': 'in gioco',
    'mw.play.starts': 'inizia',
    'mw.play.wasRole': '{name} era {role}',
    'mw.play.title': 'Indizi',
    'mw.play.hint': 'Inizia {name}, poi in senso orario: ognuno dice a voce una parola collegata alla propria.',
    'mw.play.toVote': 'Vai alla votazione',

    'mw.vote.title': 'Votazione',
    'mw.vote.hint': 'Discutete e votate, poi toccate sul tavolo chi eliminare. In caso di pareggio decide {name}.',
    'mw.vote.clues': 'Indizi',
    'mw.vote.tie': 'Pareggio',
    'mw.vote.noTie': 'Niente pareggio',
    'mw.vote.tieDecides': 'Decide {name}',
    'mw.vote.tieHint': 'La Dea della giustizia sceglie chi eliminare: toccalo sul tavolo.',
    'mw.vote.goddessEliminates': '{name} elimina',
    'mw.vote.eliminate': 'Eliminare',
    'mw.vote.guessTitle': '{name} è Mr White!',
    'mw.vote.guessBody': 'Ultima possibilità: indovina la parola dei civili per vincere.',
    'mw.vote.guessPlaceholder': 'La parola dei civili è…',
    'mw.vote.guessWrong': 'Non indovina',
    'mw.vote.guessConfirm': 'Conferma',

    'mw.results.civiliWin': 'Vincono i Civili',
    'mw.results.civiliWinSub': 'Tutti gli impostori sono stati smascherati.',
    'mw.results.mrWhiteWin': 'Vince Mr White',
    'mw.results.mrWhiteWinSub': '{name} ha indovinato la parola.',
    'mw.results.impostoriWin': 'Vincono gli Impostori',
    'mw.results.impostoriWinSub': 'Sono sopravvissuti fino alla fine.',
    'mw.results.replay': 'Rigioca',
    'mw.results.changeTable': 'Cambia tavolo',
    'mw.results.recCivili': 'Vittoria Civili',
    'mw.results.recMrWhite': 'Mr White ha indovinato',
    'mw.results.recImpostori': 'Vittoria Impostori',

    'mw.leave.title': 'Uscire dalla partita?',
    'mw.leave.body': 'Il round in corso andrà perso.',
    'mw.leave.stay': 'Resta',
    'mw.leave.exit': 'Esci',

    'hu.name': 'Heads Up',
    'hu.description': 'Telefono in fronte: indovina la parola dagli indizi.',
    'hu.menu.play': 'Gioca',
    'hu.menu.playSub': 'Nuova partita',
    'hu.menu.words': 'Parole',
    'hu.menu.wordsSub': 'Categorie',
    'hu.menu.rules': 'Come si gioca',
    'hu.menu.rulesSub': 'Regole',

    'hu.unit.words': 'parole',
    'hu.packsPlaceholder': 'Una parola per riga:\nSpiderman\nPizza\nBallare la macarena\n\n…oppure JSON: { "words": ["Spiderman", "Pizza"] }',
    'hu.packsEmpty': 'Nessuna parola valida. Scrivi una parola per riga.',

    'hu.rules.shortSection': 'In breve',
    'hu.rules.phone': 'Telefono in fronte',
    'hu.rules.phoneDesc': 'Un giocatore tiene il telefono sulla fronte, schermo verso gli altri.',
    'hu.rules.clues': 'Gli altri danno indizi',
    'hu.rules.cluesDesc': 'Descrivono la parola senza dirla, finché non la indovini.',
    'hu.rules.tilt': 'Inclina',
    'hu.rules.tiltDesc': 'Giù = indovinata, su = passo. In alternativa tocca lo schermo (destra = giusto, sinistra = passo).',
    'hu.rules.goalSection': 'Obiettivo',
    'hu.rules.goal': 'Più parole possibili',
    'hu.rules.goalDesc': 'Indovinane il più possibile prima che scada il tempo.',

    'hu.setup.noCategory': 'Nessuna categoria attiva. Aggiungine o attivane una in “Gestisci parole”.',
    'hu.setup.wordsMissing': 'Categorie da scaricare: serve la rete una volta.',
    'hu.team.n': 'Squadra {n}',
    'hu.opt.teams': 'Squadre',
    'hu.opt.teamsSummary': '{n} squadre · {split}',
    'hu.opt.nTeams': '{n} squadre',
    'hu.opt.teamsHint': 'Tocca un nome per spostarlo alla squadra dopo. Al tavolo il colore del posto dice chi sta con chi.',
    'hu.opt.teamsReset': 'Ridividi il cerchio a metà',
    'hu.opt.noPlayers': 'Non è seduto nessuno. Aggiungi giocatori al tavolo.',
    'hu.opt.words': 'Categoria',
    'hu.opt.wordsHint': 'Una sola per partita: qui la categoria è metà del gioco, e si sa da che parte cercare.',
    'hu.opt.noCategory': 'Nessuna categoria attiva',
    'hu.opt.round': 'Il giro',
    'hu.opt.roundSummary': '{s}s · {n} a testa',
    'hu.opt.rounds': 'Turni per squadra',
    'hu.opt.nRounds': '{n} a testa',
    'hu.opt.roundsHint': 'Finito il giro arriva la classifica, con Rigioca lì sotto: un altro giro costa un tocco.',
    'hu.turn.title': 'Tocca a {team}',
    'hu.turn.hint': 'Il telefono a {name}: sulla fronte, schermo verso gli altri. Gli altri della squadra danno gli indizi.',
    'hu.turn.round': 'Giro {n} di {of}',
    'hu.turn.holder': 'ha il telefono',
    'hu.turn.of': 'di {n}',
    'hu.tally.title': '{team}, tempo scaduto',
    'hu.tally.point': 'punto',
    'hu.tally.points': 'punti',
    'hu.tally.next': 'Passa il telefono',
    'hu.tally.standings': 'Classifica',
    'hu.standings.won': 'Vince {team}',
    'hu.standings.draw': 'Pareggio',
    'hu.standings.again': 'Rigioca',
    'hu.standings.change': 'Cambia qualcosa',
    'hu.setup.pickCategory': 'Scegli una categoria',
    'hu.setup.teamShort': '{team}: servono almeno {n} giocatori',
    'hu.setup.teamsShort': 'Ogni squadra ha bisogno di almeno {n} giocatori',
    'hu.menu.stats': 'Statistiche',
    'hu.menu.statsSub': 'Chi vince più spesso',
    'hu.rules.teamsSection': 'Le squadre',
    'hu.rules.teams': 'Due o più squadre',
    'hu.rules.teamsDesc': 'Al tavolo il colore del posto dice chi sta con chi. Servono almeno due giocatori per squadra: uno tiene il telefono, gli altri parlano.',
    'hu.rules.turn': 'Un turno a testa',
    'hu.rules.turnDesc': 'Ogni squadra gioca il suo turno, poi arriva la classifica. Il telefono gira dentro la squadra, così non lo tiene sempre lo stesso.',
    'rules.title': 'Come si gioca',
    'hu.setup.duration': 'Durata',
    'hu.setup.controls': 'Comandi',
    'hu.setup.tiltHint': 'Inclina il telefono in giù = giusto, in su = passo. In alternativa tocca lo schermo: destra = giusto, sinistra = passo.',
    'hu.setup.invert': 'Inverti inclinazione',

    'hu.ready.start': 'Avvia',
    'hu.countdown.go': 'Via!',

    'hu.play.hint': '← passo · giusto →',
    'hu.play.noSensorHint': 'Sensore assente — tocca: ← passo · giusto →',
    'hu.play.tapHint': 'Tocca: ← passo · giusto →',

    'hu.results.empty': 'Nessuna parola giocata.',

    'ql.name': 'Quiplash',
    'ql.description': 'Due risposte alla stessa domanda: vince chi fa ridere.',
    'ql.menu.play': 'Gioca',
    'ql.menu.playSub': 'Apri una stanza',
    'ql.menu.words': 'Domande',
    'ql.menu.wordsSub': 'Pacchetti',
    'ql.menu.rules': 'Come si gioca',
    'ql.menu.rulesSub': 'Regole',
    'ql.menu.stats': 'Statistiche',
    'ql.menu.statsSub': 'Chi vince pi\u00f9 spesso',

    'ql.unit.prompts': 'domande',
    'ql.packsPlaceholder': 'Una domanda per riga:\nIl peggior nome per un gatto\nLa scusa peggiore per non venire a cena\n\n\u2026oppure JSON: { "prompts": ["Il peggior nome per un gatto"] }',
    'ql.packsEmpty': 'Nessuna domanda valida. Scrivi una domanda per riga.',

    'ql.rules.shortSection': 'In breve',
    'ql.rules.write': 'Due risposte a testa',
    'ql.rules.writeDesc': 'Il telefono fa un giro: ognuno legge le sue due domande e risponde, senza farsi vedere.',
    'ql.rules.duel': 'Due risposte per domanda',
    'ql.rules.duelDesc': 'Ogni domanda \u00e8 andata a due persone. Al tavolo escono una sotto l\u2019altra, senza nomi.',
    'ql.rules.vote': 'Vota il tavolo',
    'ql.rules.voteDesc': 'Chi non ha scritto sceglie la pi\u00f9 divertente: tocca il suo posto una volta per la prima risposta, due per la seconda.',
    'ql.rules.pointsSection': 'I punti',
    'ql.rules.points': '100 a voto',
    'ql.rules.pointsDesc': 'Ogni voto vale 100 punti a chi ha scritto quella risposta.',
    'ql.rules.sweep': 'Quiplash!',
    'ql.rules.sweepDesc': 'Una risposta che prende tutti i voti vale il doppio. Servono almeno due votanti: a tavola in tre, un voto solo fa cappotto per forza.',
    'ql.rules.rounds': 'I giri valgono di pi\u00f9',
    'ql.rules.roundsDesc': 'Il primo giro vale una volta, il secondo il doppio, il terzo il triplo: fino all\u2019ultima domanda si pu\u00f2 ribaltare tutto.',
    'ql.rules.tableSection': 'Al tavolo',
    'ql.rules.secret': 'Chi ha scritto non vota',
    'ql.rules.secretDesc': 'I due posti che hanno scritto restano spenti: si sa chi si sfida, non chi ha scritto cosa. Si scopre appena finito il voto.',
    'ql.rules.goal': 'Obiettivo',
    'ql.rules.goalDesc': 'Arrivare in fondo con pi\u00f9 punti di tutti.',

    'ql.opt.prompts': 'Domande',
    'ql.opt.promptsHint': 'Da qui pesca la partita. Base non si apre: una domanda letta prima \u00e8 una domanda a cui hai gi\u00e0 pensato.',
    'ql.opt.baseMissing': 'Domande da scaricare: serve la rete una volta',
    'ql.opt.choose': 'Scegli un pacchetto',
    'ql.opt.nPrompts': '{n} domande',
    'ql.opt.round': 'Il giro',
    'ql.opt.oneRound': '1 giro',
    'ql.opt.nRounds': '{n} giri',
    'ql.opt.roundHint': 'Un giro sono due domande a testa, e una sfida per giocatore.',
    'ql.opt.pointsHint': 'Il secondo giro vale il doppio, il terzo il triplo.',

    'ql.setup.needPlayers': 'Servono almeno {n} giocatori seduti',
    'ql.setup.promptsMissing': 'Domande da scaricare: serve la rete una volta.',
    'ql.setup.pickPack': 'Scegli almeno un pacchetto di domande',
    'ql.setup.noPacks': 'Nessun pacchetto di domande. Scrivine uno da \u201cDomande\u201d, nel menu del gioco.',
    'ql.setup.morePrompts': 'Servono almeno {n} domande, una per giocatore',

    'ql.round.of': 'Giro {n} di {of}',

    'ql.pass.counter': 'scrivono',
    'ql.pass.kicker': 'Il telefono a',
    'ql.pass.hint': 'Due domande, due risposte. Leggile solo tu, poi passa il telefono.',
    'ql.pass.mine': 'Sono io',

    'ql.write.step': '{name} \u00b7 {n} di {of}',
    'ql.write.placeholder': 'La tua risposta\u2026',
    'ql.write.left': '{n} caratteri',
    'ql.write.next': 'Avanti',
    'ql.write.done': 'Nascondi e passa',

    'ql.duel.counter': 'domanda',
    'ql.duel.wrote': 'ha scritto',
    'ql.duel.chose': 'risposta {n}',
    'ql.duel.vote': 'voto',
    'ql.duel.votes': 'voti',
    'ql.duel.readHint': 'Leggila ad alta voce, poi scopri le due risposte.',
    'ql.duel.reveal': 'Scopri le risposte',
    'ql.duel.voteHint': 'Tocca il posto di chi vota: una volta per la prima risposta, due per la seconda, tre per togliere il voto.',
    'ql.duel.count': 'Conta i voti',
    'ql.duel.missing': 'Mancano {n} voti',
    'ql.duel.missingOne': 'Manca un voto',

    'ql.tally.nothing': 'niente',
    'ql.tally.sweep': 'QUIPLASH!',
    'ql.tally.next': 'Prossima domanda',
    'ql.tally.standings': 'Classifica',

    'ql.standings.title': 'Classifica',
    'ql.standings.won': 'Vince {name}',
    'ql.standings.draw': 'Pareggio',
    'ql.standings.again': 'Rigioca',
    'ql.standings.change': 'Cambia qualcosa',
    'ql.standings.next': 'Giro {n}',

    'ql.leave.title': 'Uscire dalla partita?',
    'ql.leave.body': 'Le risposte scritte finora andranno perse.',
    'ql.leave.stay': 'Resta',
    'ql.leave.exit': 'Esci',

    'ql.menu.join': 'Entra',
    'ql.menu.joinSub': 'In una stanza aperta',
    'ql.menu.alone': 'Un telefono solo',
    'ql.menu.aloneSub': 'Si passa in giro',

    'ql.room.codeLabel': 'Codice stanza',
    'ql.room.codeHint': 'Gli altri aprono GameHub, Quiplash, Entra, e battono questo codice.',
    'ql.room.opening': 'Sto aprendo la stanza\u2026',
    'ql.room.offline': 'Stanza non raggiungibile: serve la rete. Con un telefono solo si gioca lo stesso, dal menu.',
    'ql.room.inRoom': 'in stanza',
    'ql.room.youShort': '(tu)',
    'ql.room.mine': 'Gioco anch\u2019io',
    'ql.room.mineNo': 'solo schermo',
    'ql.room.mineTitle': 'Chi sei?',
    'ql.room.mineHint': 'Questo telefono tiene la partita. Pu\u00f2 anche giocarla: scegli con quale profilo.',
    'ql.room.begin': 'Inizia',
    'ql.room.writeTitle': 'Tutti stanno scrivendo',
    'ql.room.writing': 'scrive',
    'ql.room.written': 'ha scritto',
    'ql.room.waitingFor': 'Manca ancora {names}.',
    'ql.room.allWritten': 'Ci sono tutte.',
    'ql.room.writeMine': 'Scrivi le tue',
    'ql.room.closeWriting': 'Chiudi le risposte',
    'ql.room.voted': 'ha votato',
    'ql.room.yourVote': 'Tocca la risposta che ti fa ridere di pi\u00f9.',
    'ql.room.theirVote': 'Votano dal loro telefono. Chi ha scritto non vota.',
    'ql.room.closeVote': 'Chiudi il voto',
    'ql.room.close': 'Chiudi la stanza',

    'ql.join.title': 'Entra in una stanza',
    'ql.join.hint': 'Il codice ce l\u2019ha chi ha aperto la partita.',
    'ql.join.code': 'Codice',
    'ql.join.who': 'Chi sei',
    'ql.join.enter': 'Entra',
    'ql.join.knocking': 'Busso alla {code}\u2026',
    'ql.join.knockingHint': 'Se non entra, controlla il codice: la stanza deve essere aperta in questo momento.',
    'ql.join.inRoom': 'Ci sei',
    'ql.join.waitStart': 'Aspetta che la stanza faccia partire la partita.',
    'ql.join.out': 'Esci dalla stanza',
    'ql.join.yourTurn': 'Le tue due domande',
    'ql.join.written': 'Risposte inviate',
    'ql.join.waitDots': 'Aspetta\u2026',
    'ql.join.pick': 'Vota la pi\u00f9 divertente. Non sai chi le ha scritte.',
    'ql.join.voted': 'Votato. Aspetta gli altri.',
    'ql.join.yoursTwo': 'Una di queste \u00e8 tua: questa volta si guarda.',
    'ql.join.waitRoom': 'Aspetta la stanza\u2026',
    'ql.join.closed': 'Stanza chiusa',
    'ql.join.closedHint': 'Chi teneva la partita \u00e8 uscito. La partita finisce l\u00ec.',
    'ql.join.started': 'Partita gi\u00e0 iniziata',
    'ql.join.startedHint': 'Si entra prima che cominci. Fatti chiamare al prossimo giro.',

    'ql.duel.nOf': 'Domanda {n} di {of}',
    'ql.write.stepPlain': '{n} di {of}',
  },

  en: {
    'common.back': 'Back',
    'common.forward': 'Forward',
    'common.cancel': 'Cancel',
    'common.save': 'Save',
    'common.delete': 'Delete',
    'common.edit': 'Edit',
    'common.done': 'Done',
    'common.new': 'New',
    'common.menu': 'Menu',
    'common.restore': 'Restore',
    'common.whatItDoes': 'What it does',

    'hub.players': 'Players',
    'hub.settings': 'Settings',

    'settings.theme': 'Theme',
    'settings.dark': 'Dark',
    'settings.light': 'Light',
    'settings.theme.system': 'System',
    'settings.theme.light': 'Light',
    'settings.theme.dark': 'Dark',
    'settings.language': 'Language',
    'settings.offline': 'Offline',
    'settings.offlineSub': 'Installed · works with no network',
    'settings.version': 'Version',
    'settings.wordsRev': '{game} r{n}',
    'update.ready': 'New version',
    'update.words': 'Words updated',
    'update.wordsFailed': 'Words not updated: {reason}',
    'storage.full': 'The app\u2019s storage is full: some changes won\u2019t be saved. Player photos take up the most room.',
    'storage.fullShort': 'phone storage is full',
    'mw.opt.baseMissing': 'Base not downloaded yet: needs a network once',
    'update.action': 'Update',
    'settings.install': 'Install',
    'settings.installSub': 'Add it to your Home screen',
    'install.title': 'Add it to your Home screen',
    'install.why': 'It becomes an app: its own icon, no browser bars, and it stays put with no network.',
    'install.ios.1': 'Tap Share, at the bottom of the screen.',
    'install.ios.2': 'Scroll and pick Add to Home Screen.',
    'install.ios.3': 'Confirm with Add, top right.',
    'install.android.1': 'Open the browser menu, the three dots up top.',
    'install.android.2': 'Pick Install app, or Add to Home screen.',
    'install.desktop.1': 'Look for the install icon in the address bar.',
    'install.desktop.2': 'Or pick Install from the browser menu.',
    'install.macos.1': 'Open the Share menu, in the toolbar.',
    'install.macos.2': 'Pick Add to Dock.',
    'install.noneTitle': 'Can\u2019t install here',
    'install.none.1': 'This browser can\u2019t install web apps.',
    'install.none.2': 'No matter: it still works in here, network or not. To have it as an app, open it in Chrome, Edge or Safari.',

    'players.profile': 'Profile',
    'players.addPlayer': 'Add a player',
    'players.newTitle': 'New player',
    'players.editTitle': 'Edit player',
    'players.loadPhoto': 'Upload photo',
    'players.removePhoto': 'Remove photo',
    'players.namePlaceholder': 'Name',
    'players.color': 'Colour',
    'players.emojiOptional': 'Emoji (optional)',
    'players.nameRequired': 'A name is required.',
    'players.badImage': 'Invalid image',

    'player.perGame': 'By game',
    'player.recent': 'Recent activity',
    'player.emptyFeed': 'No games yet. Play a round to fill the feed!',
    'player.win': 'Won',
    'player.loss': 'Lost',
    'player.deleteTitle': 'Delete {name}?',
    'player.deleteBody': 'The player leaves the roster. Recorded stats stay in the games.',

    'stats.title': 'Stats',
    'stats.played': 'Games',
    'stats.won': 'Wins',
    'stats.winRate': 'Win rate',
    'stats.player': 'Player',
    'stats.playedShort': 'Played',
    'stats.wonShort': 'Won',
    'stats.empty': 'No games recorded yet. Play a round to see the stats!',
    'stats.sub': '{played} games · {won} won · {pct}%',
    'stats.emptyTitle': 'No games yet',
    'stats.emptySub': 'Play a round to fill the ranking',

    'time.now': 'now',
    'time.min': '{n}m ago',
    'time.hour': '{n}h ago',
    'time.day': '{n}d ago',

    'table.seatFree': 'Free seat',
    'table.drawerToggle': 'Open or close the drawer',
    'table.sheetClose': 'Close',
    'table.free': 'free',
    'table.seat': 'seat',
    'table.seats': 'seats',
    'table.addSeat': 'Chair',
    'table.addPlayer': 'Players',
    'table.start': 'Start',
    'table.needSeats': 'At least {n} seats needed',
    'table.whoSits': 'Who sits here?',
    'table.pickForSeat': 'Pick who sits here, or leave it free: whoever takes it introduces themselves when the phone comes round.',
    'table.pickProfile': 'Pick a profile or create a new one.',
    'table.allSeated': 'Every profile is already at the table: create a new one.',
    'table.empty': 'Empty the table',
    'table.emptySure': 'Really empty it',
    'table.playersHint': 'Tap whoever sits down: the panel stays open, so you add them one after another. The order you sort out afterwards, on the table.',
    'table.free.action': 'Clear',
    'table.remove': 'Remove',

    'packs.title': 'Words',
    'packs.section': 'Packs',
    'packs.new': '+ New pack',
    'packs.newTitle': 'New pack',
    'packs.newSub': 'Write your own words',
    'packs.icon': 'Icon',
    'packs.addRow': '+ Row',
    'packs.dropRow': 'Drop the row',
    'packs.dropRowSure': 'Really drop the row',
    'packs.rowCount': '{n} {unit}',
    'packs.halfRow': '1 row to finish',
    'packs.halfRows': '{n} rows to finish',
    'packs.halfRowStop': 'Finish or drop the half-written row.',
    'packs.halfRowsStop': 'Finish or drop the {n} half-written rows.',
    'packs.inPlay': 'Use in play',
    'packs.editTitle': 'Edit pack',
    'packs.namePlaceholder': 'Pack name',
    'packs.use': 'Use {name}',
    'packs.tagCustom': ' · yours',
    'packs.tagLocked': ' · hidden',
    'packs.locked': '{n} {unit}. The words in the base pack stay hidden, so nobody at the table knows them before playing.',
    'packs.lockedEdit': 'The base pack can\u2019t be edited.',
    'packs.tagModified': ' · edited',
    'packs.deleteSure': 'Really delete',
    'packs.deleted': 'Deleted “{name}”',
    'packs.restored': 'Put back as it was',
    'packs.saved': 'Saved “{name}” ({n} {unit})',
    'packs.added': 'Added “{name}” ({n} {unit})',
    'packs.nameRequired': 'Give the pack a name.',
    'packs.emptyText': 'The text is empty.',
    'packs.badJson': 'Invalid JSON. Check the syntax.',
    'packs.missingField': 'JSON has no valid "{field}" field.',
    'packs.notFound': 'Pack not found.',
    'packs.unit.items': 'entries',

    'mw.name': 'Mr White',
    'mw.description': 'Find the impostor who doesn’t know the word.',
    'mw.menu.play': 'Play',
    'mw.menu.playSub': 'New game',
    'mw.menu.words': 'Words',
    'mw.menu.wordsSub': 'Packs and pairs',
    'mw.menu.rules': 'How to play',
    'mw.menu.rulesSub': 'Rules and roles',
    'mw.menu.stats': 'Stats',
    'mw.menu.statsSub': 'Games and wins',

    'mw.unit.pairs': 'pairs',
    'mw.packsHelp': 'Word pairs: the civilians’ word and a similar one for the Undercovers. Tap a pack to rename it or change its pairs. Which ones to use you pick at the table, before each game.',
    'mw.packsPlaceholder': 'One pair per line:\ndog,wolf\npizza,focaccia\n\n…or JSON: { "pairs": [ { "civilian": "dog", "undercover": "wolf" } ] }',
    'mw.packsEmpty': 'No valid pair. Format: "word,similar-word" per line.',

    'mw.role.civile': 'Civilian',
    'mw.role.undercover': 'Undercover',
    'mw.role.mrwhite': 'Mr White',
    'mw.roles.civili': 'Civilians',
    'mw.roles.impostori': 'Impostors',

    'mw.rules.rolesSection': 'Roles',
    'mw.rules.civili': 'They get the secret word.',
    'mw.rules.undercover': 'They get a similar but different word.',
    'mw.rules.mrwhite': 'Gets no word at all: must fake knowing it.',
    'mw.rules.goddess': 'Goddess of Justice',
    'mw.rules.goddessDesc': 'One per round, drawn at random among whoever is still in. If the vote ties, she decides who goes. Next round another one is drawn.',
    'mw.rules.countSection': 'How many impostors',
    'mw.rules.minority': 'Always outnumbered',
    'mw.rules.minorityDesc': 'Civilians must outnumber the impostors (Undercover + Mr White): with 3–4 players 1 impostor, with 5–6 up to 2, with 7–8 up to 3, and so on.',
    'mw.rules.suggested': 'Suggested',
    'mw.rules.suggestedDesc': 'About 4 impostors per 10 players: one Mr White (two from 11 up), the rest Undercover.',
    'mw.rules.flowSection': 'How a round goes',
    'mw.rules.step0': '0 · The table',
    'mw.rules.step0Desc': 'Lay the seats out as you are sitting. Whoever takes a free seat introduces themselves when the phone arrives.',
    'mw.rules.step1': '1 · Dealing',
    'mw.rules.step1Desc': 'The phone goes round the table: everyone sees their own word in private (or finds out they are Mr White).',
    'mw.rules.step2': '2 · Clues',
    'mw.rules.step2Desc': 'In turn, clockwise, everyone says out loud one word linked to their own. Don’t write it down. A Mr White never starts.',
    'mw.rules.step3': '3 · Vote',
    'mw.rules.step3Desc': 'Talk it over and eliminate a suspect by tapping them on the table. Their role is revealed. On a tie the Goddess of Justice decides.',
    'mw.rules.winSection': 'Who wins',
    'mw.rules.winCivili': 'If they eliminate every impostor (Undercover + Mr White).',
    'mw.rules.winImpostori': 'If they last until a single civilian is left.',
    'mw.rules.winMrWhite': 'If, once eliminated, they guess the civilians’ word.',

    'mw.setup.min3': 'At least 3 players are needed.',
    'mw.setup.max20': '20 players max.',
    'mw.setup.needImpostor': 'At least one impostor is needed.',
    'mw.setup.tooMany': 'Too many impostors: civilians must outnumber them.',
    'mw.setup.pickPack': 'Pick at least one word pack',
    'mw.opt.less': 'Fewer {label}',
    'mw.opt.more': 'More {label}',
    'mw.opt.civili': 'Civilians',
    'mw.opt.characters': 'Characters',
    'mw.opt.charactersHint': 'Civilians are everyone else: whoever the impostors leave over. Tap ? to see what a role does.',
    'mw.opt.words': 'Words',
    'mw.opt.choose': 'Choose',
    'mw.opt.nPacks': '{n} packs',
    'mw.opt.packsHint': 'Pick which packs the pair is drawn from. You create and edit them from “Words” in the game menu.',
    'mw.opt.nPairs': '{n} pairs',

    'mw.extra.meme': 'Mr Meme',
    'mw.extra.memeDesc': 'Every clue round one player at random gets it: they describe their word by gesture only, no talking.',
    'mw.extra.lovers': 'The Lovers',
    'mw.extra.loversDesc': 'Two players fall in love at the start, and they know it. If one is eliminated the other follows, and only then does the table find out.',
    'mw.extra.revenger': 'The Revenger',
    'mw.extra.revengerDesc': 'When eliminated, they take another player with them, their pick. Once per round.',
    'mw.extra.needPlayers': 'Needs {n} players or more.',
    'mw.extra.section': 'On top',

    'mw.deal.loverOf': 'You are in love with {name}.',
    'mw.deal.revenger': 'You are the Revenger: if they get you, you take one with you.',

    'mw.play.meme': '{name}’s turn: describe your word by gesture only, no talking.',
    'mw.play.memeNote': 'gestures',
    'mw.play.loversOut': '{a} and {b} were lovers.',

    'mw.revenge.kicker': '{name}’s revenge',
    'mw.revenge.hint': 'Tap on the table who goes with them.',
    'mw.revenge.pick': 'Pick who',
    'mw.revenge.confirm': 'Take {name}',
    'mw.revenge.note': 'revenge',

    'mw.deal.counter': 'words',
    'mw.deal.passTo': 'Pass the phone to',
    'mw.deal.onlyYou': 'Only you should see the screen.',
    'mw.deal.seeWord': 'See your word',
    'mw.deal.youAreMrWhite': 'You are Mr White',
    'mw.deal.mrWhiteHint': 'You don’t know the word: fake it.',
    'mw.deal.yourWord': 'Your word is',
    'mw.deal.hideAndStart': 'Hide and begin',
    'mw.deal.hideAndPass': 'Hide and pass on',

    'mw.join.freeSeat': 'Free seat',
    'mw.join.who': 'Who are you?',
    'mw.join.pickOrCreate': 'Before the word, introduce yourself: tap your profile or create one.',
    'mw.join.createOnly': 'Before the word, create your profile.',
    'mw.join.createMine': 'Create my profile',

    'mw.goddess.kicker': 'Goddess of Justice',
    'mw.goddess.short': 'Goddess',
    'mw.goddess.hint': 'If the vote ties, she decides who goes.',
    'mw.goddess.start': 'Let’s start',
    'mw.goddess.round': '{name} is this round’s Goddess of Justice.',

    'mw.play.inGame': 'in play',
    'mw.play.starts': 'starts',
    'mw.play.wasRole': '{name} was {role}',
    'mw.play.title': 'Clues',
    'mw.play.hint': '{name} starts, then clockwise: everyone says out loud one word linked to their own.',
    'mw.play.toVote': 'Go to the vote',

    'mw.vote.title': 'Vote',
    'mw.vote.hint': 'Talk it over and vote, then tap on the table who goes. On a tie {name} decides.',
    'mw.vote.clues': 'Clues',
    'mw.vote.tie': 'Tie',
    'mw.vote.noTie': 'No tie',
    'mw.vote.tieDecides': '{name} decides',
    'mw.vote.tieHint': 'The Goddess of Justice picks who goes: tap them on the table.',
    'mw.vote.goddessEliminates': '{name} eliminates',
    'mw.vote.eliminate': 'Eliminate',
    'mw.vote.guessTitle': '{name} is Mr White!',
    'mw.vote.guessBody': 'Last chance: guess the civilians’ word to win.',
    'mw.vote.guessPlaceholder': 'The civilians’ word is…',
    'mw.vote.guessWrong': 'Wrong guess',
    'mw.vote.guessConfirm': 'Confirm',

    'mw.results.civiliWin': 'The Civilians win',
    'mw.results.civiliWinSub': 'Every impostor has been unmasked.',
    'mw.results.mrWhiteWin': 'Mr White wins',
    'mw.results.mrWhiteWinSub': '{name} guessed the word.',
    'mw.results.impostoriWin': 'The Impostors win',
    'mw.results.impostoriWinSub': 'They survived to the end.',
    'mw.results.replay': 'Play again',
    'mw.results.changeTable': 'Change table',
    'mw.results.recCivili': 'Civilians win',
    'mw.results.recMrWhite': 'Mr White guessed',
    'mw.results.recImpostori': 'Impostors win',

    'mw.leave.title': 'Leave the game?',
    'mw.leave.body': 'The round in progress will be lost.',
    'mw.leave.stay': 'Stay',
    'mw.leave.exit': 'Leave',

    'hu.name': 'Heads Up',
    'hu.description': 'Phone on your forehead: guess the word from the clues.',
    'hu.menu.play': 'Play',
    'hu.menu.playSub': 'New game',
    'hu.menu.words': 'Words',
    'hu.menu.wordsSub': 'Categories',
    'hu.menu.rules': 'How to play',
    'hu.menu.rulesSub': 'Rules',

    'hu.unit.words': 'words',
    'hu.packsPlaceholder': 'One word per line:\nSpiderman\nPizza\nDancing the macarena\n\n…or JSON: { "words": ["Spiderman", "Pizza"] }',
    'hu.packsEmpty': 'No valid word. Write one word per line.',

    'hu.rules.shortSection': 'In short',
    'hu.rules.phone': 'Phone on the forehead',
    'hu.rules.phoneDesc': 'One player holds the phone on their forehead, screen facing the others.',
    'hu.rules.clues': 'The others give clues',
    'hu.rules.cluesDesc': 'They describe the word without saying it, until you guess it.',
    'hu.rules.tilt': 'Tilt',
    'hu.rules.tiltDesc': 'Down = guessed, up = pass. Or tap the screen (right = correct, left = pass).',
    'hu.rules.goalSection': 'Goal',
    'hu.rules.goal': 'As many words as you can',
    'hu.rules.goalDesc': 'Guess as many as possible before time runs out.',

    'hu.setup.noCategory': 'No category is active. Add one or switch one on in “Manage words”.',
    'hu.setup.wordsMissing': 'Categories not downloaded yet: needs a network once.',
    'hu.team.n': 'Team {n}',
    'hu.opt.teams': 'Teams',
    'hu.opt.teamsSummary': '{n} teams · {split}',
    'hu.opt.nTeams': '{n} teams',
    'hu.opt.teamsHint': 'Tap a name to move it to the next team. At the table the seat colour says who plays with whom.',
    'hu.opt.teamsReset': 'Split the circle in half again',
    'hu.opt.noPlayers': 'Nobody is seated. Add players to the table.',
    'hu.opt.words': 'Category',
    'hu.opt.wordsHint': 'One per match: here the category is half the game, and everyone knows where to look.',
    'hu.opt.noCategory': 'No category is on',
    'hu.opt.round': 'The round',
    'hu.opt.roundSummary': '{s}s · {n} each',
    'hu.opt.rounds': 'Turns per team',
    'hu.opt.nRounds': '{n} each',
    'hu.opt.roundsHint': 'The standings come up at the end of the round, with Play again right under them: another round costs one tap.',
    'hu.turn.title': '{team} is up',
    'hu.turn.hint': 'Phone to {name}: on the forehead, screen facing the others. The rest of the team give the clues.',
    'hu.turn.round': 'Round {n} of {of}',
    'hu.turn.holder': 'holds the phone',
    'hu.turn.of': 'of {n}',
    'hu.tally.title': '{team}, time\u2019s up',
    'hu.tally.point': 'point',
    'hu.tally.points': 'points',
    'hu.tally.next': 'Pass the phone',
    'hu.tally.standings': 'Standings',
    'hu.standings.won': '{team} wins',
    'hu.standings.draw': 'Draw',
    'hu.standings.again': 'Play again',
    'hu.standings.change': 'Change something',
    'hu.setup.pickCategory': 'Pick a category',
    'hu.setup.teamShort': '{team}: needs at least {n} players',
    'hu.setup.teamsShort': 'Every team needs at least {n} players',
    'hu.menu.stats': 'Stats',
    'hu.menu.statsSub': 'Who wins most often',
    'hu.rules.teamsSection': 'The teams',
    'hu.rules.teams': 'Two or more teams',
    'hu.rules.teamsDesc': 'At the table the seat colour says who plays with whom. A team needs at least two players: one holds the phone, the others talk.',
    'hu.rules.turn': 'One turn each',
    'hu.rules.turnDesc': 'Every team plays its turn, then the standings come up. The phone rotates inside the team, so it is never always the same person.',
    'rules.title': 'How to play',
    'hu.setup.duration': 'Duration',
    'hu.setup.controls': 'Controls',
    'hu.setup.tiltHint': 'Tilt the phone down = correct, up = pass. Or tap the screen: right = correct, left = pass.',
    'hu.setup.invert': 'Invert tilt',

    'hu.ready.start': 'Go',
    'hu.countdown.go': 'Go!',

    'hu.play.hint': '← pass · correct →',
    'hu.play.noSensorHint': 'No sensor — tap: ← pass · correct →',
    'hu.play.tapHint': 'Tap: ← pass · correct →',

    'hu.results.empty': 'No word played.',

    'ql.name': 'Quiplash',
    'ql.description': 'Two answers to the same question: the funniest one wins.',
    'ql.menu.play': 'Play',
    'ql.menu.playSub': 'Open a room',
    'ql.menu.words': 'Prompts',
    'ql.menu.wordsSub': 'Packs',
    'ql.menu.rules': 'How to play',
    'ql.menu.rulesSub': 'Rules',
    'ql.menu.stats': 'Stats',
    'ql.menu.statsSub': 'Who wins most often',

    'ql.unit.prompts': 'prompts',
    'ql.packsPlaceholder': 'One prompt per line:\nThe worst name for a cat\nThe worst excuse for skipping dinner\n\n\u2026or JSON: { "prompts": ["The worst name for a cat"] }',
    'ql.packsEmpty': 'No valid prompt. Write one prompt per line.',

    'ql.rules.shortSection': 'In short',
    'ql.rules.write': 'Two answers each',
    'ql.rules.writeDesc': 'The phone goes round: everyone reads their own two prompts and answers them, out of sight of the others.',
    'ql.rules.duel': 'Two answers per prompt',
    'ql.rules.duelDesc': 'Every prompt went to two people. At the table they come up one under the other, with no names on them.',
    'ql.rules.vote': 'The table votes',
    'ql.rules.voteDesc': 'Whoever didn\u2019t write picks the funnier one: tap their seat once for the first answer, twice for the second.',
    'ql.rules.pointsSection': 'The points',
    'ql.rules.points': '100 a vote',
    'ql.rules.pointsDesc': 'Every vote is worth 100 points to whoever wrote that answer.',
    'ql.rules.sweep': 'Quiplash!',
    'ql.rules.sweepDesc': 'An answer that takes every vote is worth double. It needs at least two voters: at a table of three a single vote sweeps whatever it does.',
    'ql.rules.rounds': 'Later rounds are worth more',
    'ql.rules.roundsDesc': 'The first round counts once, the second double, the third triple: it can all turn around on the last prompt.',
    'ql.rules.tableSection': 'At the table',
    'ql.rules.secret': 'The two who wrote don\u2019t vote',
    'ql.rules.secretDesc': 'Their seats go dark: you know who is up against whom, not who wrote which. That comes out the moment the vote is in.',
    'ql.rules.goal': 'Goal',
    'ql.rules.goalDesc': 'Finish with more points than anybody else.',

    'ql.opt.prompts': 'Prompts',
    'ql.opt.promptsHint': 'What the match draws from. Base doesn\u2019t open: a prompt read beforehand is a prompt you have already written the answer to.',
    'ql.opt.baseMissing': 'Prompts not downloaded yet: needs a network once',
    'ql.opt.choose': 'Pick a pack',
    'ql.opt.nPrompts': '{n} prompts',
    'ql.opt.round': 'The round',
    'ql.opt.oneRound': '1 round',
    'ql.opt.nRounds': '{n} rounds',
    'ql.opt.roundHint': 'A round is two prompts each, and one duel per player.',
    'ql.opt.pointsHint': 'The second round is worth double, the third triple.',

    'ql.setup.needPlayers': 'At least {n} players have to be seated',
    'ql.setup.promptsMissing': 'Prompts not downloaded yet: needs a network once.',
    'ql.setup.pickPack': 'Switch on at least one pack of prompts',
    'ql.setup.noPacks': 'No pack of prompts. Write one from \u201cPrompts\u201d, in the game\u2019s menu.',
    'ql.setup.morePrompts': 'At least {n} prompts are needed, one per player',

    'ql.round.of': 'Round {n} of {of}',

    'ql.pass.counter': 'writing',
    'ql.pass.kicker': 'Phone to',
    'ql.pass.hint': 'Two prompts, two answers. Only you read them, then pass the phone on.',
    'ql.pass.mine': 'That\u2019s me',

    'ql.write.step': '{name} \u00b7 {n} of {of}',
    'ql.write.placeholder': 'Your answer\u2026',
    'ql.write.left': '{n} characters',
    'ql.write.next': 'Next',
    'ql.write.done': 'Hide and pass',

    'ql.duel.counter': 'prompt',
    'ql.duel.wrote': 'wrote',
    'ql.duel.chose': 'answer {n}',
    'ql.duel.vote': 'vote',
    'ql.duel.votes': 'votes',
    'ql.duel.readHint': 'Read it out loud, then show the two answers.',
    'ql.duel.reveal': 'Show the answers',
    'ql.duel.voteHint': 'Tap a voter\u2019s seat: once for the first answer, twice for the second, three times to take the vote back.',
    'ql.duel.count': 'Count the votes',
    'ql.duel.missing': '{n} votes still missing',
    'ql.duel.missingOne': 'One vote still missing',

    'ql.tally.nothing': 'nothing',
    'ql.tally.sweep': 'QUIPLASH!',
    'ql.tally.next': 'Next prompt',
    'ql.tally.standings': 'Standings',

    'ql.standings.title': 'Standings',
    'ql.standings.won': '{name} wins',
    'ql.standings.draw': 'Draw',
    'ql.standings.again': 'Play again',
    'ql.standings.change': 'Change something',
    'ql.standings.next': 'Round {n}',

    'ql.leave.title': 'Leave the match?',
    'ql.leave.body': 'The answers written so far will be lost.',
    'ql.leave.stay': 'Stay',
    'ql.leave.exit': 'Leave',

    'ql.menu.join': 'Join',
    'ql.menu.joinSub': 'A room somebody opened',
    'ql.menu.alone': 'One phone only',
    'ql.menu.aloneSub': 'Passed around',

    'ql.room.codeLabel': 'Room code',
    'ql.room.codeHint': 'The others open GameHub, Quiplash, Join, and type this code.',
    'ql.room.opening': 'Opening the room\u2026',
    'ql.room.offline': 'The room can\u2019t be reached: it needs a network. One phone passed around still plays, from the menu.',
    'ql.room.inRoom': 'in the room',
    'ql.room.youShort': '(you)',
    'ql.room.mine': 'I\u2019m playing too',
    'ql.room.mineNo': 'screen only',
    'ql.room.mineTitle': 'Who are you?',
    'ql.room.mineHint': 'This phone is holding the match. It can play it as well: pick which profile.',
    'ql.room.begin': 'Start',
    'ql.room.writeTitle': 'Everybody is writing',
    'ql.room.writing': 'writing',
    'ql.room.written': 'written',
    'ql.room.waitingFor': 'Still waiting on {names}.',
    'ql.room.allWritten': 'They\u2019re all in.',
    'ql.room.writeMine': 'Write yours',
    'ql.room.closeWriting': 'Close the answers',
    'ql.room.voted': 'voted',
    'ql.room.yourVote': 'Tap the one that makes you laugh more.',
    'ql.room.theirVote': 'They vote from their own phones. Whoever wrote doesn\u2019t vote.',
    'ql.room.closeVote': 'Close the vote',
    'ql.room.close': 'Close the room',

    'ql.join.title': 'Join a room',
    'ql.join.hint': 'Whoever opened the match has the code.',
    'ql.join.code': 'Code',
    'ql.join.who': 'Who you are',
    'ql.join.enter': 'Join',
    'ql.join.knocking': 'Knocking on {code}\u2026',
    'ql.join.knockingHint': 'If it doesn\u2019t let you in, check the code: the room has to be open right now.',
    'ql.join.inRoom': 'You\u2019re in',
    'ql.join.waitStart': 'Wait for the room to start the match.',
    'ql.join.out': 'Leave the room',
    'ql.join.yourTurn': 'Your two prompts',
    'ql.join.written': 'Answers sent',
    'ql.join.waitDots': 'Hold on\u2026',
    'ql.join.pick': 'Vote for the funnier one. You don\u2019t know who wrote them.',
    'ql.join.voted': 'Voted. Waiting for the others.',
    'ql.join.yoursTwo': 'One of these is yours: this one you just watch.',
    'ql.join.waitRoom': 'Waiting for the room\u2026',
    'ql.join.closed': 'Room closed',
    'ql.join.closedHint': 'Whoever was holding the match left. That\u2019s where it ends.',
    'ql.join.started': 'The match has started',
    'ql.join.startedHint': 'You join before it begins. Get yourself called in for the next one.',

    'ql.duel.nOf': 'Prompt {n} of {of}',
    'ql.write.stepPlain': '{n} of {of}',
  }
}

function normalize(value) {
  const l = String(value || '').slice(0, 2).toLowerCase()
  return LANGS.includes(l) ? l : null
}

// Your choice if you made one, otherwise English — whoever opens GameHub
// without touching anything gets the language most people at a party can read.
// The phone's own language is deliberately NOT consulted: an Italian phone
// would land on Italian, which is the opposite of a default.
let lang = normalize(storage.get(KEY, null)) || 'en'

const listeners = new Set()

export function getLang() { return lang }
export function getLocale() { return LOCALES[lang] || 'it-IT' }
export function langName(l = lang) { return LANG_NAMES[l] || l }

export function setLang(next) {
  const l = normalize(next)
  if (!l || l === lang) return lang
  lang = l
  storage.set(KEY, lang)
  if (typeof document !== 'undefined') document.documentElement.lang = lang
  for (const fn of listeners) fn(lang)
  return lang
}

// The settings bead: one tap moves to the next language, then wraps.
export function cycleLang() {
  return setLang(LANGS[(LANGS.indexOf(lang) + 1) % LANGS.length])
}

export function onLangChange(fn) {
  listeners.add(fn)
  return () => listeners.delete(fn)
}

export function t(key, vars) {
  const table = DICT[lang] || DICT.it
  let s = table[key]
  if (s == null) s = DICT.it[key]
  if (s == null) return key
  if (!vars) return s
  return s.replace(/\{(\w+)\}/g, (m, name) => (vars[name] == null ? m : String(vars[name])))
}

if (typeof document !== 'undefined') document.documentElement.lang = lang
